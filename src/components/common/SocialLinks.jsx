...
/* ── Variant: "grid" — used in Contact section ───────────────────── */
function SocialGrid() {
    ...
    <div className="grid grid-cols-3 gap-3 sm:grid-cols-3">
      {socials.map((social, i) => (
        <motion.a
          ...
          whileHover={{ y: -3 }}
          className={`group flex flex-col items-center gap-2.5 rounded-2xl border p-4 transition-all duration-300 ${
            isDark
...
/* ── Variant: "row" — used in Hero / Footer ──────────────────────── */
function SocialRow({ className = "" }) {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  // Show only the most important ones in the row
  const rowSocials = socials.filter((s) =>
    ["github", "linkedin", "telegram", "email"].includes(s.id)
  );

  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {rowSocials.map((social) => (
        <a
          key={social.id}
          href={social.url}
          target={social.id === "email" ? "_self" : "_blank"}
